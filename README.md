# Amazon SQS to AWS Lambda with a DLQ

The SAM template deploys a Lambda function, an SQS queue, an SQS Dead Letter Queue to place messages that error out, and the IAM permissions required to run the application. SQS invokes the Lambda function when new messages are available.
The Lambda function will throw an exception if the message from SQS has the word _error_ in it. The SQS Queue is configured with a redrive
policy of three attempts. If the message is not procesed after three attempts, the message will be placed in the dead letter queue. 
...
```
2. To test the dead letter queue functionality, try the following:
```bash
aws sqs send-message --queue-url ENTER_YOUR_SQS_QUEUE_URL --message-body "Test error message"
```
